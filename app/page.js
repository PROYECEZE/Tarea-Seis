...
export default function Home() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = cardData.filter(card => 
      ...
      card.abstract.toLowerCase().includes(query)
    )
    ...
    <>
      ...
      <form onSubmit={handleSearch} className='flex items-center'>
        ...
        {filteredCards.length === 0 && (
          <p className="text-center text-lg text-gray-600 mt-10">Uh oh.. "{searchQuery}"</p>
        )}

        {filteredCards.length > 0 && filteredCards.length < cardData.length && (
          <p className="text-center text-lg text-gray-600 mt-10">Showing {filteredCards.length} result(s) "{searchQuery}"</p>
        )}

        {filteredCards.length === cardData.length && (
